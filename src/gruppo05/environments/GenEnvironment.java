...
public class GenEnvironment<T> implements Environment<T> {
	...
	protected Map<VarIdent, T> resolve(VarIdent id) 
	{
		for (Map<VarIdent, T> scope : scopeChain) {
			if (scope.containsKey(id)) { //containsKey(Object key) Returns true if this map contains a mapping for the specified key.
				return scope;
			}
		}
	...
	{
		return map.put(requireNonNull(id), requireNonNull(payload)); //put(K key, V value) Associates the specified value with the specified key in this map (optional operation).
	}
